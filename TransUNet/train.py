"""
This file includes the training function for the model.
To use it, please configure wandb and log in, which is used for logging the training process.
...
def train_next_day_fire(
    ...
    if skip_eval is not True:
        val_loader = DataLoader(
            val_set, shuffle=False, drop_last=True, batch_size=val_batch_size, num_workers=8
        )
    else:
        val_loader = None
    # Initialize logging
            ...
            for i, data in enumerate(train_loader):
                ...
                with torch.autocast(
                    ...
                    if use_checkpointing is True:
                        ...
                        print("train mask_pred shape: ", masks_pred[0], masks_pred.shape)
                    if masks_pred.shape != true_masks.shape:
                        print("change predict shape")
                        masks_pred = torch.randn(batch_size, 1, 64, 64)
...
def evaluate(
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=False):
        for i, batch in tqdm(
            ...
            # predict the mask
            mask_pred = net(image)
            # print("evaluation mask_pred shape: ", mask_pred, mask_pred.shape)
            # print("evaluation mask_true shape: ", mask_true, mask_true.shape)
